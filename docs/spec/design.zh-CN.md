...
依赖任务是否可调度，必须对 `TaskState` **做穷尽划分**：任何一个既不算"放行"也不算"拦截"的状态，都会让该任务永远停在 pending —— 而调度循环一旦发现没有就绪任务就会退出，于是这个任务被静默地从本次运行中丢掉，没有 attempt 记录、没有事件、没有报错，运行本身还显示成功。

| 依赖状态 | 对下游任务的影响 |
|---|---|
| `completed`、`partial` | 满足，调度它。部分证据也是证据，且 `produced_context` 两种情况下都会传给 worker。 |
| `pending`、`ready`、`running` | 尚未结束，等下一轮。 |
| 其余状态（`failed`、`timeout`、`blocked`、`cancelled`） | 无法满足，把下游标记为 `blocked`。 |

第三行应当由前两行取补集推导得出，而不是逐个列举，这样将来新增 `TaskState` 时不会重新打开同一个缺口。
